...
import { Calendar, MapPin, DollarSign, CloudRain, Clock, ArrowLeft, Download, Share2, Edit, Coffee, Utensils, Bed, Camera } from 'lucide-react';
...
function ItineraryPage() {
  ...
  const Directions = ({ origin, destination }) => (
    <div className="directions-fallback">
      <p>Directions from {origin} to {destination}</p>
      <p className="text-sm text-gray-500">Directions service unavailable in mock mode</p>
    </div>
  );
  ...
  useEffect(() => {
    const generateItinerary = async () => {
      ...
      // fallback timer,if API doesn't respond in 10 seconds, use mock data
      const fallbackTimer = setTimeout(() => {
        console.warn('Gemini API response taking too long. Falling back to mock data.');
        useMockData();
      }, 10000); // 10 sec timeout for fallback
      ...
      try {
        ...
        if (!apiKey) {
          console.warn('Gemini API key is missing. Using mock data instead.');
      ...
      const data = response.data;
        ...
        try {
          ...
          console.warn("Falling back to mock data due to parsing failure.");
          useMockData();
          return;
        }
        ...
        if (!itinerary || !itinerary.days || !Array.isArray(itinerary.days)) {
          console.warn('Invalid itinerary data format. Falling back to mock data.');
          useMockData();
          return;
        }
        ...
        // Clear the fallback timer if an error occurs
        clearTimeout(fallbackTimer);
    ...
    const useMockData = () => {
        ...
        const activities = [
          {
            ...
            title: "Breakfast",
            description: `Enjoy a delicious breakfast at a local café in ${formData.destination}.`,
            location: getCoordinates(),
            type: "restaurant",
            cost: 15 + Math.floor(Math.random() * 10)
          },
          {
            time: "10:00 AM",
            title: `${formData.destination} Walking Tour`,
            description: `Explore the beautiful sights of ${formData.destination} on foot.`,
            location: getCoordinates(),
            type: "attraction",
          ...
          },
          {
            ...
            title: "Lunch",
            description: "Try the local cuisine at a recommended restaurant.",
            location: getCoordinates(),
            type: "restaurant",
            cost: 20 + Math.floor(Math.random() * 15)
          },
          {
            time: "03:00 PM",
            title: dayNum % 2 === 0 ? "Museum Visit" : "Local Shopping",
            description: dayNum % 2 === 0 ? 
              `Visit a renowned museum in ${formData.destination}.` : 
              `Shop for local goods and souvenirs in ${formData.destination}.`,
            location: getCoordinates(),
            type: "attraction",
            cost: 15 + Math.floor(Math.random() * 25)
          },
          {
            time: mealTimes.dinner,
            title: "Dinner",
            description: "Enjoy dinner at a highly-rated restaurant.",
            location: getCoordinates(),
            type: "restaurant",
            cost: 30 + Math.floor(Math.random() * 20)
          }
        ];
        
        // Add a special activity on some days
        if (dayNum % 3 === 0) {
          activities.push({
            time: "09:00 PM",
            title: "Evening Entertainment",
            description: `Experience local nightlife in ${formData.destination}.`,
            location: getCoordinates(),
            type: "attraction",
            cost: 35 + Math.floor(Math.random() * 25)
          });
        }
      ...
      const weatherConditions = [
        "Sunny", "Partly Cloudy", "Cloudy", "Light Rain", "Rainy"
      ];
      ...
      const mockItinerary = {
        ...
        weather: Array.from({ length: totalDays }, (_, i) => {
  ...
  useEffect(() => {
    if (mapApiLoaded && itineraryData && !map && window.google) {
      const currentDayActivities = itineraryData.days.find(day => day.day === activeDay)?.activities || [];
      
      // Find the first activity with a location to center the map
      const centerActivity = currentDayActivities.find(activity => activity.location);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Our AI is planning your trip to {formData?.destination || 'your destination'}.</p>
        </div>
      </div>
    );
  }
  ...
  if (!itineraryData) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-semibold text-gray-800 mb-2">No itinerary data available</h2>
          <button 
            onClick={onBack}
      ...
      <div className="bg-white shadow-md">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="text-center flex-1">
            <h1 className="text-xl md:text-2xl font-bold text-gray-800">Your {itineraryData.tripType} Trip to {itineraryData.destination}</h1>
            <p className="text-sm text-gray-600">{formatDate(itineraryData.startDate)} - {formatDate(itineraryData.endDate)}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
              ...
              <div className="space-y-4">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{itineraryData.totalDays} Days</div>
                  </div>
                </div>
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{itineraryData.costEstimate.low} - {itineraryData.costEstimate.high} {itineraryData.costEstimate.currency}</div>
                  </div>
                </div>
            ...
            <motion.div
              ...
              variants={fadeIn}
            >
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Your Itinerary</h2>
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-sm text-gray-500">{getDayDate(day.day)}</div>
                      </div>
          ...
          <div className="lg:col-span-2">
            ...
            >
              {itineraryData.days.filter(day => day.day === activeDay).map((day) => (
                <div key={day.day}>
                  <div className="flex justify-between items-center mb-6">
                    <div>
                      <h2 className="text-2xl font-bold text-gray-800">Day {day.day}: {day.title}</h2>
                          ...
                          <div className="flex justify-between">
                            ...
                            {activity.cost > 0 && (
                              <div className="text-sm text-gray-700">
                                <span className="font-medium">{activity.cost} {itineraryData.costEstimate.currency}</span>
                              </div>
                            )}
                          </div>
